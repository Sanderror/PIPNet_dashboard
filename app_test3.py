...
def render_predictions_tab():
    ...
    for prediction_idx, (cls, score) in enumerate(preds, start=1):
        ...
        st.markdown(f"### {prediction_idx}. {display_cls} — Prediction Strength: {score:.3f}")
        ...
        if rects:
            rects_total_score = sum(
                extract_patch_info(r.name)["mul"]
                for r in rects
                if extract_patch_info(r.name)["mul"] is not None
            )
            ...
            for idx, img_path in enumerate(rects, start=1):
                ...
                with col_stats:
                    if all(v is not None for v in info.values()):
                        ...
                        updated_percent = (updated_mul / rects_total_score) * 100 if rects_total_score else 0
                        ...
                        percent_w = (
                            updated_weight / total_prototype_weight * 100
                        ...
                        st.markdown(
                            ...
                            <div style="text-align: right; font-size: 12px; color: gray; margin-top: 4px;">
                                Total Prediction Strength: {rects_total_score:.1f}
                            </div>
                            """,
                            unsafe_allow_html=True,
                        )
                ...
                current_percentage = (
                    class_prototype_weight / total_prototype_weight * 100
                    if total_prototype_weight
                    else 0
                )
...
if selected_tab == "About":
    ...
    - Applies a **importance** (how important the reference feature is for recognizing that class). All image region's weighted similarities are summed to calculate the final prediction strength.
